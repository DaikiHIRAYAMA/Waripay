...
function caluculate() {
    ...
    const { groupname, grouptype } = JSON.parse(
    ...
    eventList.forEach(({ payerlist, receiverlist }) => {
        ...
        if (grouptype.balance == "equal") {
            ...
            const rate =
                grouptype.ratelist[grouptype.balance == "boy++" ? 2 : 1];
            let boyGirl = [0, 0];
            receiverlist.forEach((receiverid) =>
                memberList.find(({ memberid }) => memberid == receiverid)
                    .membertype == "male"
                    ? boyGirl[0]++
                    : boyGirl[1]++
            );
        ...
        memberList.forEach(({ memberid, membertype }, index) => {
            if (receiverlist.includes(memberid)) {
                memberList[index].pay +=
                    membertype == "male" ? boyPay : girlPay;
            }
            memberList[index].pay -= Number(
                (
                    payerlist.find(
                        (payerData) => payerData.memberid == memberid
                    ) || {
                        cost: 0,
                    }
                ).cost
    ...
    while (payerIndex < receiverIndex) {
        if (
            (memberList[receiverIndex].pay += memberList[payerIndex].pay) >= 0
        ) {
            showCalculation(
    ...
    function showCalculation(payerName, receiverName, money) {
        ...
        if (fixedMoney) {
            ...
            cloneItem.querySelector(".payment-cost").textContent =
                fixedMoney + "円";
